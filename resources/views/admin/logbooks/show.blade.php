...
<div class="mb-6 flex flex-wrap items-start justify-between gap-4">
    <div class="space-y-2 text-sm">
        ...
        <p>
            ...
            <span class="text-ink-soft">{{ $vacancy?->title ?? '—' }}</span>
        </p>
    </div>
    <div class="flex flex-wrap items-center gap-2">
        @if ($entries->isNotEmpty())
            <a href="{{ route('admin.logbooks.export', ['user' => $user, 'program_id' => $programId]) }}"
...
@if ($entries->isEmpty())
    <div class="card-soft p-10 text-center">
        ...
        <p class="mt-2 text-sm text-ink-soft">Peserta belum mengisi logbook{{ $programId ? ' untuk lowongan ini' : '' }}.</p>
    </div>
@else
                        ...
                        <tr class="border-t border-ink/8 {{ $loop->even ? 'bg-slate-50 dark:bg-white/5' : 'bg-panel' }}">
                            ...
                            <td class="px-5 py-4">
                                @if ($entry->attachment_path)
                                    <a href="{{ route('admin.logbooks.attachment', $entry) }}" target="_blank" rel="noopener" class="block w-24">
                                        <img src="{{ route('admin.logbooks.attachment', $entry) }}" alt="Dokumentasi logbook"
                            ...
                            <td class="px-5 py-4">
                                ...
                                <details class="mt-1">
                                    ...
                                    <form method="POST" action="{{ route('admin.logbooks.review', $entry) }}" class="mt-2 max-w-xs space-y-2">
                                        @csrf
