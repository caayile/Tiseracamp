...
@php
    $googleData = session('google_oauth_data');
    $isGoogleFlow = filled($googleData);
@endphp
...
@if ($isGoogleFlow)
    <div class="mt-6 rounded-2xl border border-brand/20 bg-brand-mist/40 px-4 py-3 text-sm text-ink-soft">
        <p class="font-semibold text-brand-mid">Daftar dengan Google</p>
        <p class="mt-1">Lengkapi data berikut untuk menyelesaikan pendaftaran. Setelah ini kamu bisa login menggunakan Google.</p>
    </div>
@endif
...
<form method="POST" action="{{ route('register') }}" class="mt-8 space-y-4">
    ...
    <div class="grid grid-cols-2 rounded-2xl bg-slate-100 p-1">
        <label class="cursor-pointer">
            <input type="radio" name="role" value="student" class="peer sr-only" @checked(old('role', $googleData['role'] ?? 'student') === 'student')>
        ...
        <label class="cursor-pointer">
            <input type="radio" name="role" value="mentor" class="peer sr-only" @checked(old('role', $googleData['role'] ?? '') === 'mentor')>
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="text" name="name" value="{{ old('name', $googleData['name'] ?? '') }}" class="w-full rounded-xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/25" placeholder="Nama lengkap Anda" required {{ $isGoogleFlow ? 'readonly' : '' }}>
        </div>
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="email" name="email" value="{{ old('email', $googleData['email'] ?? '') }}" class="w-full rounded-xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/25" placeholder="name@example.com" required {{ $isGoogleFlow ? 'readonly' : '' }}>
        </div>
        @error('email') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>

    <div id="expertise-field" class="{{ old('role', $googleData['role'] ?? '') === 'mentor' ? '' : 'hidden' }}">
    ...
    <button type="submit" class="w-full rounded-xl bg-brand py-3.5 text-sm font-bold text-white shadow-lg shadow-brand/30 transition hover:bg-brand-dark">
        {{ $isGoogleFlow ? 'Daftar & Hubungkan Google' : 'Register' }}
    </button>
</form>
...
<p class="mt-6 text-center text-sm text-ink-soft">
    {{ $isGoogleFlow ? 'Sudah punya akun?' : 'Sudah punya akun?' }}
